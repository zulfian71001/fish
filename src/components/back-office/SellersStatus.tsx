...
import { get_request_seller } from "@/GlobalRedux/features/sellerReducer";
...
const SellersStatus = () => {
  ...
  const {loader, sellers, totalSellers} = useAppSelector((state)=>state.seller)
      ...
      <div className="flex flex-col gap-4">
        ...
        </div>
        {
              ...
              <tbody>
                {
                  sellers?.map((data:any, i:number)=>(
                    <tr className="bg-cyan-500 border-b dark:bg-gray-800 text-white  dark:border-gray-700" key={i}>
          ...
          <div className="w-full text-center">
            <p className="text-white font-semibold">Tidak ada Request seller</p>
          </div>
          )
        }
