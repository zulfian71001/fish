...
const DetailSeller = () => {
  ...
  return (
    <section className="h-screen flex justify-between p-8 rounded-xl space-y-6 bg-slate-800">
      <div className="flex flex-col p-2 gap-6 w-1/4">
        <h3>Sellers</h3>
        <div className="flex flex-col">
          {iUser.map((data, i) => (
            <div
              key={i}
              className="flex items-center gap-4 border border-slate-900 p-2 rounded-md hover:bg-slate-900"
              onClick={() => activeUserClick(i)}
            >
              <div className="relative">
                ...
                />
                <div
                  className={`w-3 h-3 rounded-full bg-green-400 absolute bottom-1 right-0 `}
                ></div>
              </div>
      ...
      <div className="flex flex-col p-2 gap-4 w-3/4">
        {iUser
