...
const EditProduct = ({ productId }: { productId: string }) => {
  ...
  const [showImage, setShowImage] = useState<string[]>(product ? product.images : []);
  ...
  const handleImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setShowImage((prevShowImage) => [...prevShowImage, ...imageUrls]);
    }
  };
  ...
  const changeImage = (image: File, idx: number) => {
    const tempImages = [...images];
    const tempShowImage = [...showImage];
    ...
    if (image) {
      ...
      // Update showImage array with new URL
      const imageUrl = URL.createObjectURL(image);
      tempShowImage[idx] = imageUrl;
      setShowImage(tempShowImage);
  
      // Call update_product_image
      console.log({
        productId ,// Pass the productId
        oldImage: product.images[idx], // Pass the old image to be replaced
        newImage: image, // Pass the new image
      })
  ...
  const updateProduct = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch(update_product(dataProduct));
  };
