...
import { messageClear } from "@/GlobalRedux/features/sellerReducer";
...
const OrdersUser = () => {
  ...
  const { shipping_fee, successMsg, errorsMsg } = useAppSelector(
    (state) => state.seller
  );
    ...
    <section className="w-full flex flex-col justify-between p-8 rounded-xl space-y-6 bg-slate-50">
      {userInfo.status != "active" ? (
        <div className="w-full flex flex-col justify-center items-center text-slate-700">
          ...
          <p>
            Silahkan lengkapi data profil anda dan menunggu aktivasi dari admin
          </p>
        </div>
      ) : (
        <>
